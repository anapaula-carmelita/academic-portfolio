# 📚 Pesquisa Aplicada e Portfólio Acadêmico

Bem-vindo(a) ao meu repositório dedicado à investigação conceitual, metodologias quantitativas e design de experiências de aprendizagem tecnológica. 

Este espaço documenta projetos que unem o rigor analítico à aplicação prática, refletindo a minha visão de como a tecnologia pode solucionar problemas complexos e educar.

## 🎯 Eixos de Atuação

Meu trabalho e pesquisa estão fundamentados em três pilares principais:

*   🧠 **Foco em IA e Didática:** Projetos de investigação aplicada que exploram a interseção entre a Inteligência Artificial, a literacia digital e o ensino da computação.
*   📊 **Foco Analítico e Matemático:** Investigações conceituais abrangendo desde avaliações matemáticas na Teoria dos Jogos até estratégias avançadas de Engenharia de Prompts.
*   💼 **Foco Corporativo:** Documentação de projetos focados em soluções práticas de IA, métodos quantitativos e no design de experiências instrucionais para o mercado de tecnologia.

## 📂 Projetos em Destaque

Abaixo estão os principais estudos e documentações hospedados neste repositório:

### 1. Inteligência Artificial na Educação: Engenharia de Prompts
*   **Descrição:** Estudo aplicado sobre a utilização de Large Language Models (LLMs) para o planejamento educacional. O projeto explora técnicas de Engenharia de Prompts (contextualização, definição de limites e critérios) para gerar roteiros de ensino de lógica de programação (estruturas condicionais e laços no Scratch).
*   **Acesse o projeto:** [Link para a pasta/arquivo]
<!--
     2. Teoria dos Jogos: O Teorema do Sacrifício
*   **Descrição:** Modelagem conceitual voltada para a Teoria da Decisão. O projeto analisa matematicamente cenários clássicos, como o Dilema do Prisioneiro, avaliando as estratégias, os pesos das decisões e o equilíbrio entre os agentes envolvidos.
*   **Acesse o projeto:** [Link para a pasta/arquivo]
-->
*(Mais projetos serão adicionados continuamente...)*

## 🛠️ Temas e Tecnologias Abordados

*   **Inteligência Artificial:** Engenharia de Prompts, RAG, Comportamento de LLMs.<!--**Matemática e Lógica:** Teoria dos Jogos, Estatística, Modelagem de Decisão.-->
*   **Educação Tecnológica:** Pensamento Computacional, Didática Aplicada, Acessibilidade.

## 📬 Contato e Redes

Sinta-se à vontade para explorar os diretórios, ler as documentações e entrar em contato para trocarmos ideias sobre IA, tecnologia e métodos quantitativos!
*   [LinkedIn](https://www.linkedin.com/in/anacarmelitana/)
